...
export class AuthClient {
  ...
  async refreshToken(options?: RefreshTokenOptions): Promise<TokenResponse> {
    ...
    // 步骤 2: 构建请求体 { refresh_token: string }
    // 步骤 3: 调用 this.httpClient.post('/api/v1/auth/refresh', body, { skipAuth: true })
    // 步骤 4: 保存到 tokenStorage.setTokens(response)
    // 步骤 5: 更新 authStore.updateAccessToken(response.access_token, response.expires_in)
  ...
  async logout(options?: LogoutOptions): Promise<void> {
    // 实现待补充（task-developer）
    // 步骤 1: 解构选项 { silent = false, clearLocalState = true }
    // 步骤 2: 如果 !silent，调用后端登出端点（TODO）
    // 步骤 3: 如果 clearLocalState，清除 tokenStorage.clearTokens()
    // 步骤 4: 如果 clearLocalState，清除 authStore.clearAuth()
    throw new Error('Not implemented');
  }
}
...
export function createAuthClient(
  ...
  // 步骤 1: 动态导入依赖（避免循环依赖）
  // 步骤 2: 合并默认配置和用户配置
  // 步骤 3: 返回 new AuthClient(finalOptions)
  throw new Error('Not implemented');
}
